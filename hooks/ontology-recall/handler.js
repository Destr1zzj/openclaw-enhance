...
async function extractAutoSave(userMsg, assistantResp) {
  ...
  // ── Pattern 1: skill 安装 ──────────────────────────────────────────────────
  // 匹配：安装xxxskill / skillhub install xxx / 装一个xxx技能 / npm install xxx / pip install xxx / uv add
  ...
  if (/决定[用选]|采用[方案策略]|选择[了]|规则.*更新|规范.*更新|POLICY|policy.*修改|规则.*修改|改成|确认.*这样|就这么做|就这么定|这样定|改成.*了|确定.*方案|决策.*是|定了|拍板|拍下来|确定下来|最终.*方案|最终.*决定|就这么办|按.*来|按.*执行|执行.*方案/i.test(userMsg)) {
  ...
  // ── Pattern 5: 偏好表达 ───────────────────────────────────────────────────
  // 匹配：我喜欢 / 偏好 / 想要 / 不要 / 不想 / 更喜欢 / 倾向于 / 我比较喜欢 / 觉得xxx好
  ...
  // ── Pattern 7: Bug 发现 ─────────────────────────────────────────────────
  // 匹配：发现bug / 遇到报错 / 程序崩溃了 / 有个bug / 复现了 / 修复了xxx / 修了个bug / 控制台报错 / 有个问题 / 出问题了 / error / 异常
  ...
  // ── Pattern 8: 好想法/灵感 ──────────────────────────────────────────────
  // 匹配：好想法 / 灵感 / 突然想到 / 有个点子 / 冒出一个想法 / 突发奇想 / 脑洞 / 思路打开了 / 迸发了 / 想到个招
  ...
  // ── Pattern 10: 部署/上线 ──────────────────────────────────────────────
  // 匹配：部署完成 / 上线了 / 发布版本 / deploy完成 / 发版 / 版本发布 / 推送生产 / 部署到生产 / 正式环境 / 切正式 / 灰度发布 / 预发布
  ...
  // ── Pattern 11: 测试完成 ────────────────────────────────────────────────
  // 匹配：测试通过 / 用例写完 / 跑通了 / 测试过了 / 测试完成 / 验证通过 / 单测通过 / 集成测试通过 / 全量测试通过 / 自动化测试 / 冒烟测试 / 回归测试
